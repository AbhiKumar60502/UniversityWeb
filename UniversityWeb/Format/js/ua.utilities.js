	...
	UA.Utilities = {
		...
		removeCookie: function (name, multihost) {
			...
			if (multihost && position !== -1) {
				while (position !== -1) {
					UA.Utilities.createCookie(name, '', -1, '/', hostname);
					hostname = hostname.substring(position + 1);
					position = hostname.indexOf(".");
				}
				return;
			}
			UA.Utilities.createCookie(name, '', -1);
		},
		...
		url: function (src) {
			var url = '/ual/' + UA.AppData.Data.Session.LanguageCulture + '/' + UA.AppData.Data.Session.POS + '/' + src;
			return url.toLowerCase();
		},
		updateSecuCookieData: function (response) {
			var crisp = $(response).find('#CoookieCrisp'),
				cval, params, array, name, secu, exp;

			if (crisp.length > 0) {
				$(crisp).find('input').each(function () {
					cval = $(this).val();
					params = UA.Utilities.parseKeyValueString(cval);
					if ($(this).data('text')) {
						params = cval;
						array = false;
					}
					...
					UA.Utilities.createCookie(name, params, exp);
				});
			}
			//$(crisp).html('');
		},
		removeSecuData: function () {
			//UA.Utilities.removeCookie("User", true);
			//UA.Utilities.removeCookie("Session", true);
			//UA.Utilities.removeCookie("uasession", true);
		},

...
Date.getMonthNumberFromName = function (name) {
	var n = Date.CultureInfo.monthNames, m = Date.CultureInfo.abbreviatedMonthNames, ni = Date.CultureInfo.abbreviatedMonthNamesInvariant, s = name.toLowerCase();
	for (var i = 0; i < n.length; i++) {
		if (ni[i].toLowerCase() == s || n[i].toLowerCase() == s || m[i].toLowerCase() == s) {
			return i; 
		}
	}
	return -1;
...
Date.getDayNumberFromName = function (name) {
	var n = Date.CultureInfo.dayNames, m = Date.CultureInfo.abbreviatedDayNames, ni = Date.CultureInfo.abbreviatedDayNamesInvariant, o = Date.CultureInfo.shortestDayNames, s = name.toLowerCase();
	for (var i = 0; i < n.length; i++) { 
		if (ni[i].toLowerCase() == s || n[i].toLowerCase() == s || m[i].toLowerCase() == s) {
			return i; 
		}
